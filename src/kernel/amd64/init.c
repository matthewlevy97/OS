	...
	.gdt_page = {
		.gdt = {
			[GDT_ENTRY_KERNEL32_CS]         =	\
				GDT_ENTRY_INIT(					\
					(uint64_t)(SEGMENT_PRESENT | SEGMENT_DB | SEGMENT_CODE), \
					0, 0xFFFFF),
			[GDT_ENTRY_KERNEL_CS]           =	\
				GDT_ENTRY_INIT(					\
					(uint64_t)(SEGMENT_PRESENT | SEGMENT_LONG_MODE | SEGMENT_CODE),\
					0, 0xFFFFF),
			[GDT_ENTRY_KERNEL_DS]           =	\
				GDT_ENTRY_INIT(					\
					(uint64_t)(SEGMENT_PRESENT | SEGMENT_DB | SEGMENT_DATA), \
					0, 0xFFFFF),
			[GDT_ENTRY_DEFAULT_USER32_CS]   =	\
				GDT_ENTRY_INIT(					\
				(uint64_t)(SEGMENT_PRESENT | SEGMENT_DB | SEGMENT_CODE | SEGMENT_USER), \
				0, 0xFFFFF),
			[GDT_ENTRY_DEFAULT_USER_DS]	    =	\
				GDT_ENTRY_INIT(					\
				(uint64_t)(SEGMENT_PRESENT | SEGMENT_DB | SEGMENT_DATA | SEGMENT_USER), \
				0, 0xFFFFF),
			[GDT_ENTRY_DEFAULT_USER_CS]	    =	\
				GDT_ENTRY_INIT(					\
				(uint64_t)(SEGMENT_PRESENT | SEGMENT_LONG_MODE | SEGMENT_CODE | SEGMENT_USER), \
				0, 0xFFFFF),
			[GDT_ENTRY_TSS]	    = \
				TSS_ENTRY_INIT()
		}
...
// TODO: This doesn't support multiple CPU cores currently
struct cpu_local_data *arch_get_current_cpu()
{
	return &base_cpu_data;
}